...
class ForgotPasswordController extends Controller
{
    public function index()
    {
        ...
        return view('user.auth.forgot-password', compact(
            'page_title',
        ));
    }
    ...
    public function forgotPassword(Request $request)
    {
        ...
        if ($user){
            session()->put('reset-email', $user->email);
            sendOtp($user->email, $message);
        }

        return response()->json(['message' => "Password reset request has been sent to your account email"]);
    }
