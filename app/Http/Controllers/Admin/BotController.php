...
class BotController extends Controller
{
    ...
    public function edit(Request $request)
    {
        ...
        if (!$bot) {
            return response()->json(validationError('Plan not found'), 422);
        }
        ...
        return response()->json(['message' => 'Plan updated successfully successfully']);
    }
    ...
    public function manageActivation(Request $request)
    {
        ...
        if (!$bot) {
            return response()->json(validationError('Plan activation instead not found'), 422);
        }

        $message = 'Action not valid';
        if ($action == 'delete') {
            $bot->delete();
            $mesage = 'Plan activation has been deleted successfully';
        } elseif ($action == 'reactivate') {
            $bot->status = 'active';
            $bot->save();
            $message = 'Plan reactivated succesfully';
        } elseif ($action == 'suspend') {
            $bot->status = 'suspended';
            $bot->save();
            $message = 'Plan suspended succesfully';
        }
